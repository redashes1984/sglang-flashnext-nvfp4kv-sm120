...
## Results (stack generations: baseline → round-2 tuned → nvfp4kv-1m trial → expert cold pool → round-5 freeze → round-6 fp8kv-1M live → round-7 nvfp4kv-1M final → round-8 top-k route → round-9 fp8kv on CT113 + estfix → round-10 CT110 pennyroyal-v2.5.1 live: gcfix + MXFP8 + cold pool keep330+64 + cache-report)
...
**Upstream watch (mid-term, 1–2 weeks): track #38144.** Its value is fixing the mechanism inside the original kernel with regression tests attached — more thorough for the whole library; for this deployment it only makes the fork base cleaner. Once a fix lands on `qwen4-main-squashed`, rebase the base directly; keep the routing branch — it's harmless and degrades into a redundant fallback on its own. In the same window #37893 / #37941 compete for the same fix slot: do nothing until upstream converges on one.
...
## Tenth round (2026-09-17): CT110 pennyroyal-v2.5.1 live — gcfix + online MXFP8 + cold pool keep330+64 + cache-report

**Context**: the CT110 box runs the pennyroyal fork (`ghcr.io/jpezzulli/sglang-rtxpro6000` lineage, editable install at `/opt/pennyroyal`, unit `sglang-dealignai-fp8kv-hicache.service`, port 8000). It took the fork's fast-forward to tag `pennyroyal-v2.5.1` (quoted tool-marker parser fix, checkpoint-donation pressure strategy, QSA chunked-prefun unaligned-prefix fix, C6 profile) and re-applied our seven local overlay edits on top via `git stash --autostash` with zero conflicts. Overlay diff archived verbatim at `patches/pennyroyal-v2.5.1-overlay.diff`; cold-pool module at `patches/expert_cold_pool.py` (md5 `6d17690934bfcfb1587f2db88b52141f`).

**gcfix, corrected against round 9**: the CT113 tree already carried `gc.collect()` inside `_profile_available_bytes`, but the pennyroyal CT110 tree did NOT — its `kv_cache_configurator.py` profiles with `empty_cache` only, so the profiled KV pool oscillated run-to-run (observed 368,576 ↔ 451,776 on identical hardware). Adding one `gc.collect()` before the availability read (module import + call site in `get_available_gpu_memory` path) stabilized the profiled number at 451,776. Lesson generalizes across forks: the round-9 "gcfix already exists upstream" finding is **tree-dependent — grep the live tree before believing either verdict**. Rollback anchor: `kv_cache_configurator.py.bak-gcfix-20260917`.

**Online MXFP8** (`Environment=SGLANG_SM120_ONLINE_MXFP8=true`): FlashInfer CUTLASS + UE8M0 scales take the eligible BF16 projections to fp8 at load; the KV pool rose 451,776 → **693,056** on top of gcfix. Journal signatures: `Flash-Next online FP8 enabled on SM120` then `MXFP8 projection ready` per QKV/MergedColumn/Row/Replicated module. Backup `.bak-mxfp8-20260917`.

**Cold pool keep330+64 closes the 1M gap**: with `SGLANG_EXPERT_KEEP_MASK=config/expert_keep_330_final.json`, `SGLANG_EXPERT_KEEP_OFFLOAD=1`, `SGLANG_EXPERT_COLD_POOL_SLOTS=64`, the shrink moved ~24.15 GB of expert weights to pinned host and the FP8 KV pool hit the pinned YAML cap in full — **max_total_tokens 1,572,864** (journal `KV Cache is allocated … #tokens: 1572864`), versus 693,056 pool-capped without the cold pool. HiCache host side came up at 827,008 tokens / 11.01 GB. Staging loop verified live: `calls=64 strong=82077 staged=611 evicted=0 h2d=1.69GB`, zero MISMATCH/stage_fail. This supersedes round 9's +120 K/+300 K expansion pricing for THIS tree: the pennyroyal fork's fp8kv stack reaches the pinned pool with headroom (avail ≈4.4 GB post-allocation), the CT113 crash arithmetic was specific to the int8-ckpt + PLE-pinned stack on that box. Note the HiCache asymmetry that journal now warns about: host pool (827 K) < device pool (1.57 M) — L2 covers ~53% of device; raise `hicache-ratio` if long-session re-reads get slow.

**Quality gates after the whole stack**: 188 K-token two-needle recall exact (`City: Yunling City / PIN: AZURE-9182`), short chat PINs pass, TTFT 224–240 ms, cold decode ≈109 tok/s / hot ≈92–97. One behavioral note worth repeating across machines: the keep-mask flattens the router distribution over surviving experts, and long reasoning chains measurably tend toward filler loops ("hmm hmm") — keep values at ~320+ of 512 experts; dropping below was previously falsified at CT114 (semantic battery required; byte-identity is not sufficient).

**cache-report for frontends**: the YAML carries `enable-cache-report: true` — responses then expose `usage.prompt_tokens_details.cached_tokens` (hit ratio = cached/prompt). Two parser facts from the tree itself (`usage_processor.py`): the details object is omitted entirely when cached==0 (check for `null`, not `{}`), and counting is page-quantized (`page_size: 64`), so sub-page tails and short prompts legitimately report less-than-full hits.
