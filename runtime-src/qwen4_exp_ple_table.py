...
# ---------------------------------------------------------------------------
# B+ : skip-if-valid sidecar.
#
# The file backend used to rewrite the whole table (47.7 GiB on this model)
# through the weight loader on EVERY boot -- NVMe wear and boot latency. The
# sidecar records a per-source-file stat fingerprint (path/dev/ino/size/
# mtime_ns/ctime_ns, vLLM PR #54129 style) plus the exact table identity.
# When the table file is the right size and every recorded source still
# matches, the loader skips the copies entirely; any drift rewrites the whole
# table (cheap: ~21 s at 2.3 GB/s on this box).
#
# Durability order (crash-safe):
#   fresh write -> msync(table) -> fsync(dir) -> atomic rename(sidecar)
# A crash between msync and sidecar rename simply leaves the old/absent
# sidecar -> next boot rewrites. Never the reverse (sidecar newer than data).
# ---------------------------------------------------------------------------

SIDECAR_NAME = "ple_table_sidecar.json"


def ple_sidecar_path(table_dir: str, tag: str) -> str:
    return os.path.join(table_dir, f"{tag}.{SIDECAR_NAME}")


def ple_fingerprint(model_path: str) -> Dict[str, tuple]:
    """stat() identity for every safetensors file under ``model_path``."""
    out = {}
...
def ple_sidecar_is_valid(
    sidecar_file: str,
    table_file: str,
    expected_bytes: int,
    table_shape: tuple,
    table_dtype: str,
    source_fingerprints: Dict[str, tuple],
    model_path: str,
) -> bool:
    """True only when the on-disk table provably matches the current
    checkpoint sources. False on ANY drift, missing or malformed sidecar."""
    try:
    ...
    if not isinstance(sc, dict):
        return False
    if sc.get("table_bytes") != int(expected_bytes):
        return False
    if tuple(sc.get("table_shape", ())) != tuple(table_shape):
        return False
    if sc.get("table_dtype") != str(table_dtype):
        return False
    if sc.get("model_path") != os.path.realpath(model_path):
        return False
    try:
        if os.path.getsize(table_file) != int(expected_bytes):
            return False
    except OSError:
        return False
    recorded = sc.get("sources", {})
    if set(recorded) != set(source_fingerprints):
        return False
    for name, fp in source_fingerprints.items():
        if tuple(recorded.get(name, ())) != tuple(fp):
            return False
    return True


def ple_msync(table: torch.Tensor) -> bool:
    """MS_SYNC the whole table mapping so sidecar sealing is crash-safe.

    ``from_file`` mappings are page-aligned and the data_ptr is the mapping
    base, so msync(ptr, nbytes, MS_SYNC) covers every dirtied page.
    """
    ...
    try:
        rc = libc.msync(
            ...
            ctypes.c_int(_MS_SYNC),
        )
    except Exception:
        return False
    if rc != 0:
        logger.warning("PLE table: msync failed (errno %d)", ctypes.get_errno())
        return False
    return True


def ple_sidecar_write(
    sidecar_file: str,
    expected_bytes: int,
    table_shape: tuple,
    table_dtype: str,
    ...
    """Seal after the table data has been msynced by the caller."""
    payload = {
        "version": 1,
        "model_path": os.path.realpath(model_path),
        "table_bytes": int(expected_bytes),
        "table_shape": list(table_shape),
        "table_dtype": str(table_dtype),
        "sources": {k: list(v) for k, v in source_fingerprints.items()},
    ...
    dfd = os.open(os.path.dirname(sidecar_file) or ".", os.O_RDONLY)
    try:
        os.fsync(dfd)
    finally:
        os.close(dfd)
