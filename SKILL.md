...
```
Reference numbers for the round-4 cold-pool stack (same GPU, this sglang build), post-warmup acceptance bench 2026-09-10 (steps=2/draft3/thr1.0, two independent harness runs): **C1 ≈122–129 tok/s, C12 aggregate ≈704–710 tok/s**, prefill ≈9.7K tok/s fresh-prefix (higher on radix hits), accept len 2.07–2.31 / rate 0.54–0.65. These supersede the earlier warm-cache-window figures (146–171 / 773–825) — measure after warmup completes, run twice. If your numbers are far off, check the eight tuning items (below) and the cold-pool arm line are all present. Round-3 reference (spec-off, conc6, pool 1.18M): C1 ≈75–90 / C6 ≈72–91. Round-2 (spec-on 768K): C1 ≈136 / C6 ≈550–575. fp8kv scheme: C1 ≈165 / C6 aggregate ≈355–365 tok/s (conc4, spec-on, steps=3).

Why round-4 C1 (≈122–129) now sits just below fp8kv (≈165): the round-3 gap was mostly the spec-off tax, not gather-dequant alone — the cold pool's freed VRAM funds NEXTN again at 1M. The shallow-MTP A/B (steps=1 ties on throughput, ~116/696) initially looked like a free 2.8 GB win; the steps=2 re-baseline after full warmup showed steps2 actually edges steps1 (C1 122–129 vs 116, accept len 2.07–2.31 vs 1.74–1.79), so steps=2 stays the default and steps=1 is a memory-pressure fallback only. Log-reading gotchas: `gen throughput` in the first decode line after an idle gap (or a new request's first window) is polluted by prefill/sleep-on-idle in the denominator — it can read 0.5 tok/s while the true rate is 136; trust consecutive-window values or a wall-clock curl timing, never the first line, and never compare across warmup states. Greedy byte-level output is nondeterministic **even with the cold pool fully OFF** (atomics/cuBLAS runtime property, proven by OFF-control) — never gate on raw byte equality.
