...
3. **Place configs.** YAMLs → `/opt/sglang-config/`. The shipped units hardcode `--model-path /mnt/GLOWAY_YCT2TNVMe_202732/models/…` — edit it (and `chat-template:` in the YAMLs) to your model dir; baseline unit files use `${MODEL_DIR}` but systemd does NOT expand variables inside YAML content anyway. Copy the chat template into the model dir. Ship `scripts/frspec_map_64k.pt` + `config/expert_keep_330_final.json` next to the YAMLs. Keep `served-model-name` verbatim if downstream routers key on it.
